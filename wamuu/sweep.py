...
# Returns list of lists of edges and cable index.
def sweep(instance, starting_turbine: int, clockwise: bool, tpg: int):
    ...
    node_cableindex = put_cables(power, instance.cables)
    edges = []
    for eg in edges_group: edges.extend(eg)
    for group in groups: edges.append([group[0], 0])
    
    return [[(edge[0], edge[1]), node_cableindex[edge[0]]] for edge in edges]
